...
#     # x, t, k = implicit.implicit("Trapezoidal", eval_f, initial_conditions, param_dict, gen_eval_u_cos(T), t_start, t_end, 0.05, False, False, eval_Jf=eval_Jf, use_GCR=False)
#     t_final = datetime.now()
#     total_duration = t_final - t_init
#     ic(total_duration.total_seconds())
...
#     data_fe = np.load('periodic_ss_simulation_20251130_202325.npz', allow_pickle=True)
#     x_fe = data_fe['x']
#     t_fe = data_fe['t']  
#     n_fe = int(data_fe['n'])
#     dt_fe = np.unique(np.round(np.diff(t_fe), decimals=2)).item()
#     T_fe = float(data_fe['T'])
#     parameters_fe = data_fe['params'].item()
#     input_ampl_fe = float(data_fe['input_ampl'])
#     initial_positions_fe = data_fe['initial_positions']
#     initial_velocities_fe = data_fe['initial_velocities']
#     duration_fe = float(data_fe['duration'])
#     ic(duration_fe)
#     ic(dt_fe)
...
#     # Forward Euler
#     target_times_fe = np.arange(t_fe[0], t_fe[-1] + T_fe/2, T_fe)
#     period_indices_fe = np.array([np.argmin(np.abs(t_fe - tt)) for tt in target_times_fe], dtype=int)
#     period_indices_fe = np.unique(period_indices_fe)  # remove any accidental duplicates
#     period_states_fe = x_fe[:, period_indices_fe]
#     # Get the position and velocity differences between each car at each period
#     period_positions_fe = period_states_fe[0::2, :]
#     period_velocities_fe = period_states_fe[1::2, :]
#     period_delta_positions_fe = np.diff(period_positions_fe, axis=1)
#     period_delta_velocities_fe = np.diff(period_velocities_fe, axis=1)
#     period_delta_states_fe = np.vstack((period_delta_positions_fe, period_delta_velocities_fe))
#     steady_state_periods_fe = 0
#     diffs_fe = np.diff(period_delta_states_fe, axis=1)
#     max_diffs_fe = np.max(np.abs(diffs_fe), axis=0)
...
#     confidence_fe = np.max(np.abs(period_delta_states_fe[:, -1] - period_delta_states_ref[:, -1]))
#     ic(confidence, confidence_fe)

#     plt.figure()
#     plt.semilogy(max_diffs, 'o-r')
#     plt.semilogy(max_diffs_fe, 'o-b')
#     # Horizontal line for confidence
#     plt.axhline(y=confidence, color='r', linestyle='--', label='Confidence Level')
#     plt.axhline(y=confidence_fe, color='b', linestyle='--', label='Confidence Level FE')
...
#     # Plot one period of the reference simulation to verify periodicity (last period)
#     fig, axs = plt.subplots(2, 1, figsize=(10, 8)) 
#     start_time = t_ref[-1] - T_ref
#     end_time = t_ref[-1]
#     mask = (t_ref >= start_time) & (t_ref <= end_time)
#     t_period = t_ref[mask]
#     x_period = x_ref[:, mask]
#     pos_lead_ref = initial_positions_ref[-1] + initial_velocities_ref[-1] * t_period
#     # Plot every k-th car to avoid clutter
#     k = max(1, n_ref // 10)
#     for i in range(0, n_ref, k):
#         axs[0].plot(t_period, x_period[2*i, :] - pos_lead_ref, label=f'Position car {i+1} relative to unperturbed leading car')
#     axs[0].set_xlabel('Time (s)')
#     axs[0].set_ylabel('Relative Position (m)')
#     axs[0].set_title('Relative Position vs Time (Reference Simulation)')      
#     # Plot velocities of the cars over time
#     for i in range(0, n_ref, k):
#         axs[1].plot(t_period, x_period[2*i + 1, :], label=f'Velocity car {i+1}')
#     axs[1].set_xlabel('Time (s)')
#     axs[1].set_ylabel('Velocity (m/s)')
#     axs[1].set_title('Velocity vs Time (Reference Simulation)')        
#     plt.tight_layout()
#     # Save the figure as a PNG file and a PDF file
#     fig.savefig('periodic_ss_simulation_reference_period.png')
#     fig.savefig('periodic_ss_simulation_reference_period.pdf')  
